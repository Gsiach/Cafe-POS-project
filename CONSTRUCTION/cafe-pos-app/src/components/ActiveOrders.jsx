...
export default function ActiveOrders({ session }) {
  ...
  async function fetchOrders() {
    ...
    if (error) console.error("Active orders fetch error:", error)
    else setOrders(data)
    setLoading(false)
  }
  ...
  async function markServed(orderId) {
    ...
    if (error) console.error("Mark served error:", error)
  }
  ...
  return (
    <div style={{ padding: "20px" }}>
      <h2>Active Orders — Ready to Serve</h2>
      {orders.length === 0 && <p>No orders ready.</p>}
      ...
      <div style={{ display: "flex", flexWrap: "wrap", gap: "16px" }}>
        {orders.map(o => (
          <div key={o.id} className="panel" style={{ minWidth: "260px" }}>
            ...
            <button onClick={() => markServed(o.id)} style={{ padding: "8px 14px" }}>
              Mark Served
            </button>
          </div>
        ))}
